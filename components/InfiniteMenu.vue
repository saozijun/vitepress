<template>
  <div class="infinite-menu-container">
    ...
    <template v-if="activeItem">
      ...
      <p
        :class="['face-description', { inactive: isMoving, active: !isMoving }]"
      >
...
void main() {
    ...
    vec2 st = vec2( vUvs.x, 1.0 - vUvs.y); // 只反转 y 轴
    st = (st - 0.5) * scale + 0.5;
      ...
      this.items.map(
        (item) =>
          new Promise((resolve) => {
            ...
            img.src = new URL(`/notes/img/${item.image}`, import.meta.url).pathname;
          })
      )
    ).then((images) => {
...
const handleActiveItem = (index) => {
  const validItems = props.items.length ? props.items : defaultItems;
  activeItem.value = validItems[index % validItems.length];
};
...
onMounted(() => {
  ...
  const itemsToUse = props.items.length ? props.items : defaultItems;
